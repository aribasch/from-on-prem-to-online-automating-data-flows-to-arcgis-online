...
#                    relationships.                                          #
# v2.1.5, Oct 2024 - Patch to correct overwrite failure due to missing       #
#                    thumbnail backup.                                       #
#                  - Added mirroring to swap logic when 'noProps' is set.    #
#                  - Patched restore property logic to turn off Async call   #
#                    when simply "touching" the service/view properties.     #
##############################################################################
    ...
    if not noProps:
        ...
        if hasattr( manager, "properties"):
            ...
            if dryRun:
                ...
                status = _asyncJob( manager, "updateDefinition", { "f": "json", "async": bool(serviceProperties), "updateDefinition": json.dumps(obj=serviceProperties, separators=(',', ':'))}, verbose=True if maxVerbose else (None if verbose else False))
    ...
    <touchTimeSeries>: (optional) 'Touch' Time Series enabled Layers in Feature Service and related Views,
      ...
      <preserveProps>: (optional) True or False, indicate if Service and View properties should be Preserved as
            ...
            <noProps>: (optional) True or False, indicate that NO Service or View Properties should be applied
                                  following a successful update. View's Layers are mirrored from Target service.
                                  * Caution * If set to True, 'preserveProps' setting CANNOT be set to True!
        ...
        # If Successful Update or no update and no errors
            ...
            if not outcome[ "success"] == False:
                ...
                if not (status and isinstance( status, dict) and "success" in status and status[ "success"]):
                    ...
                    if noProps:
                        print(" * Mirroring View layers from Target!")

                    # Add Updated Layer details back in!
                    for applyGroup, applyLayers in [[ "Adding Target", addLayers if viewLayers[ "layers"] and not noProps else altLayers], [ "Restoring Original", fallbackLayers]]:
                        if not applyLayers:
                            continue
                        ...
                        if dryRun:
                            ...
                            if createInfo and dataItem and not dataItem.type == "Service Definition":
                                ...
                                try:
                                    ...
                                    print('Enacting editor tracking info')
                                    infoData = {
...
if __name__ == "__main__":
    ...
    if len( sys.argv) < 4 or help:
        ...
        print( "                            a successful update. This Defaults Service or View back to its Published state! If Swapping")
        print( "                            Layers, the View's Layers are mirrored from Target service!")
